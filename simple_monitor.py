...
def monitor():
    ...
    while True:
        try:
            ...
            print("=" * 80)
            print(f"🚀 DERIBIT PRICE COLLECTOR - LIVE MONITOR")
            print(f"⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            print("=" * 80)

            # Статус системы
            print("\n📊 SYSTEM STATUS:")
            print(f"  • API: {'✅ Healthy' if health.get('status') == 'healthy' else '❌ Unhealthy'}")
            ...
            if prices.get('data'):
                for price in prices['data'][:5]:
                    time_str = price['timestamp'].split('T')[1][:8] if 'timestamp' in price else 'N/A'
                    print(
